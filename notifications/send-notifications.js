...
function buildGymReminder(store){
  const day = store.gym && store.gym.day;
  const isRest = GYM_REST_DAYS.includes(day);
  if(isRest){
    return { id:'gym_day', hour:0, minute:0, title:'Lola — يوم راحة 🌙', body:'النهاردة يوم راحة في جدولك — خد وقتك وارتاح كويس.' };
  }
  const label = GYM_DAY_LABELS[day];
  return {
    id:'gym_day', hour:0, minute:0,
    title:'Lola — تمرين النهاردة 💪',
    body: label ? `متنساش تمرين ${label} النهاردة.` : 'افتح Lola وشوف تمرين النهاردة.',
    url:'./?tab=gym'
  };
}
...
const REMINDERS = [
  ...
  { id: 'water_1',       hour: 3,  minute: 0, title: 'Lola — ميه 💧',          body: 'وقت كوباية ميه — متنساش تشرب.' },
...
async function main(){
  ...
  // تذكير الجيم بيتبني هنا (مش جوه REMINDERS الثابتة) لأنه محتاج يقرأ
  // store.gym.day الأول عشان يعرف يقول التمرين المناسب.
  const allReminders = [...REMINDERS, buildGymReminder(store)];
