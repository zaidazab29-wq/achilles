...
const REMINDERS = [
  ...
  { id: 'journal',       hour: 22, minute: 0,  title: 'Lola — احكيلي عن يومك ✍️', body: 'قبل ما يخلص يومك، سجّل يومياتك — حتى لو سطرين، بيفرقوا معايا وبيفرقوا معاك.', url:'./?tab=journal' }
];

// تذكيرات المستخدم المخصصة (اللي بتتضاف من reminder modal في index.html).
// دي مش زي REMINDERS فوق (مواعيد يومية ثابتة) — كل واحدة منها ليها تاريخ
// ووقت محدد مرة واحدة، ومتخزنة في نفس مستند Firestore تحت
// store.customReminders = { [id]: {title,date,time,due,done,pushSent} }.
// بنفس منطق الـ "id" اللي التطبيق بيستخدمه محليًا (رقم فريد لكل تذكير).
//
// الـ dedupe هنا مش عن طريق تاريخ اليوم (زي pushLastSent فوق) لأن كل تذكير
// بيحصل مرة واحدة بس؛ بدل كده بنستخدم فلاج pushSent جوه نفس التذكير، وبنعمل
// claim ليه بـ transaction قبل ما نبعته (بالظبط زي فكرة pushLastSent، بس على
// مستوى التذكير نفسه مش تاريخ اليوم).
//
// CUSTOM_MAX_AGE_MS: لو التذكير فات معاده بفترة طويلة جدًا (مثلاً السيرفر كان
// واقف أو الـ secrets اتعطلت أيام) بنسيبه من غير ما نبعته متأخر جدًا — أحسن من
// إن يوصل إشعار "تذكير" لحاجة كانت المفروض تحصل من أسبوع.
const CUSTOM_MAX_AGE_MS = 24 * 60 * 60 * 1000; // 24 ساعة

function fmtCustomDueCairo(dueMs){
  try{
...
async function sendCustomReminders(db, docRef, store, webpush, subs){
  const customReminders = store.customReminders || {};
  const now = Date.now();
  const testId = (process.env.TEST_CUSTOM_REMINDER_ID || '').trim();
  let currentSubs = subs; // بيتحدّث لو أجهزة اتشالت (subscription منتهية) عشان الحلقات اللي بعدها تستخدم النسخة الصح

  for(const [id, r] of Object.entries(customReminders)){
    const isTest = testId && String(id) === testId;

    if(!isTest){
      if(r.done || r.pushSent) continue;
      if(typeof r.due !== 'number') continue;
      if(r.due > now) continue;              // لسه معادها ما جاش
      if(now - r.due > CUSTOM_MAX_AGE_MS) continue; // قديم جدًا، سيبه
    }

    const claimed = await db.runTransaction(async (tx)=>{
      const freshSnap = await tx.get(docRef);
      const freshData = freshSnap.data() || {};
      const freshR = (freshData.customReminders || {})[id];
      if(!freshR) return false;
      if(!isTest && (freshR.done || freshR.pushSent)) return false;
      tx.update(docRef, { [`customReminders.${id}.pushSent`]: true });
      return true;
    });

    if(!claimed){
      console.log(`Skipping custom reminder "${id}" — already claimed/sent or removed.`);
      continue;
    }

    console.log(`Sending custom reminder "${id}" ("${r.title}") to ${currentSubs.length} device(s)...`);
    const payload = JSON.stringify({
      title: r.title,
      body: 'تذكير — ' + fmtCustomDueCairo(r.due),
      tag: 'reminder-' + id,
      reminderId: Number(id) || id,
      requireInteraction: true,
      actions: [
        { action: 'done', title: '✓ تم' },
        { action: 'snooze', title: '⏰ أجل 10 دقايق' }
      ]
    });
    ...
    currentSubs = stillValid;
    await docRef.update({ pushSubscriptions: currentSubs });
    console.log(`Done: custom reminder "${id}" sent.`);
  }
...
async function main(){
  ...
  // تذكيرات reminder modal المخصصة (مش مرتبطة بوقت اليوم زي فوق — كل واحدة
  // ليها معاد خاص بيها، فبتتفحص لوحدها بمعزل عن نافذة الـ 45 دقيقة اللي فوق).
  await sendCustomReminders(db, docRef, store, webpush, subs);
