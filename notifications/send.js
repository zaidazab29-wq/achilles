...
//   2. Checks whether we're inside one of the 3 daily reminder windows.
//   3. Skips it if that reminder was already sent today (dedupe), so running
//      every 15 min doesn't send the same message 4 times in an hour.
...
const REMINDERS = [
  { id: 'breakfast',     hour: 15, minute: 0, title: 'ACRO — الفطار 🍳',    body: 'سجّلت فطارك النهاردة؟ افتح ACRO وحدّث بناء الوجبات.' },
  { id: 'lunch_workout', hour: 21, minute: 0, title: 'ACRO — الغدا والتمرين 🏋', body: 'اتابعت الغدا والتمرين؟ متنساش تعلّم العادات في الداشبورد.' },
  { id: 'dinner_meds',   hour: 1,  minute: 0, title: 'ACRO — العشا والدوا 💊', body: 'قبل ما تنام: سجّلت العشا وأخدت الدوا؟' }
];

const WINDOW_MINUTES = 20; // مدى السماح — لو الـ cron اتأخر شوية، لسه هيلحق الميعاد
...
async function main(){
  ...
  for(const r of REMINDERS){
    ...
    const payload = JSON.stringify({ title: r.title, body: r.body, tag: 'acro-'+r.id, url: '/' });
